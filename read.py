...
class RFID(object):
    ...
    def util(self):
        ...
        try:
            ...
            return None
"""
End of MFRC522 RFID library
"""

"""
Start of read and check
"""
rdr = RFID()

#Pulls hashed UID from local text file and assigns to variable
file = open("hash.txt","r")
uidLocalHash = file.read()
file.close()
...
#Will endlessly loop, always ready to unlock/lock door
while True:
    
  #Calls function to hold system in loop until the MFRC522 has betected a card in range
  rdr.wait_for_tag()

  #Collects error information about the card, as well as it's type
  (error, tag_type) = rdr.request()
  
  if not error:

    #Initiates reading of NFC card UID and errors during reading
    (error, uid) = rdr.anticoll()

    #If the hash is valid, the lock is opened. In the case of this demonstration
    #a dialog is displayed
    if not error:
        
      #Hashes the read UID using HMAC with SHA-512 and six charecter salt, for
      #100000 cycles. The same proccess as the local UID went through
      uidHashed = hashlib.pbkdf2_hmac('sha512', str(uid), 'Xve7b', 100000)

      #Comparative statement checks hashes
      if uidHashed == uidLocalHash:
        if doorLocked == True:
          doorLocked = False
          root.update()
          root.withdraw()
          tkMessageBox.showinfo('NFC', 'Door Unlocked')
          root.update()

        elif doorLocked == False:
          doorLocked = True
          root.update()
          root.withdraw()
          tkMessageBox.showinfo('NFC', 'Door Locked')
          root.update()

        #Stops the crypotgraphic procedure of the reader
        rdr.stop_crypto()
      else:
        root.update()
        root.withdraw()
        tkMessageBox.showinfo('NFC', 'Invalid key. Please try again...')
        root.update()
        rdr.stop_crypto()
            
    else:
      rdr.stop_crypto()
      print('UID read error!')
        
  else:
    rdr.stop_crypto()
    print('Identity read error!')
    
# Calls GPIO cleanup
rdr.cleanup()
"""
End of reader and check
"""
